...
def build_model(hp):
    model = Sequential()
    model.add(LSTM(1,return_sequences=True, input_shape=(X_train.shape[1],X_train.shape[2])))
    layers = hp.Int('num_layers', 0, 4)
    print("layers are : " ,layers)
    for i in range(layers):
        with hp.conditional_scope('num_layers', list(range(i, 4))):
            model.add(LSTM(hp.Int(f'layer{i}_units',min_value=1,max_value=50,step=10),return_sequences=True))
    model.add(LSTM(hp.Int('last_layer_neurons',min_value=1,max_value=50,step=10)))
